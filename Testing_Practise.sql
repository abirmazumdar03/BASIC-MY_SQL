#A SCRIPT TO TEST AND PRACTISE SQL SCRIPTS
# TO LEARN AND GROW

USE ADMISSION;

CREATE DATABASE TEST_AND_FUN;

USE TEST_AND_FUN;

CREATE TABLE TABLE_1
(
DATE_TIME DATETIME UNIQUE,
NAME VARCHAR(50),
COURSE VARCHAR(10),
PHONE_NUMBER INT8 UNIQUE,
DOB DATE
);

SHOW TABLES;

ALTER TABLE TABLE_1
ADD CONSTRAINT
UNIQUE (COURSE);

SELECT * FROM TABLE_1;

#NOW ADD AN EXTRA COLUMN WITH UNQOUE CONSTRAINT
ALTER TABLE TABLE_1
ADD COLUMN ADHAR_NUMBER INT8 UNIQUE;


#MODIFY THE DATATYPE OF DATE_TIME COLUMN TO DATE ONLY
ALTER TABLE TABLE_1
MODIFY COLUMN DATE_TIME DATE;

#RENAME THE NAME COULMN TO FULL_NAME
ALTER TABLE TABLE_1
RENAME COLUMN NAME TO FULL_NAME;

SELECT * FROM TABLE_1;

#MOVE THE COURSE COLUMN TO THE LAST
ALTER TABLE TABLE_1
MODIFY COLUMN COURSE VARCHAR(10)
AFTER ADHAR_NUMBER;

#DELETE THE COULMN DATE_TIME FROM TABLE_1
ALTER TABLE TABLE_1
DROP COLUMN DATE_TIME;

ALTER TABLE TABLE_1
DROP COLUMN ADHAR_NUMBER;
# ******************************************************************************************************************
#ADDING ROWS IN THE TABLE

INSERT INTO TABLE_1
VALUES
("BARON",9065785544,"2002-06-02","BCA"),
("ABIR",9012785544,"2002-06-12","BTECH"),
("BEN",9064785544,"2002-06-16","BCOM"),
("RAY",9065787544,"2002-06-22","BBA");

#INSERTING SEPECIFC COLUMN VALUES
INSERT INTO TABLE_1 (FULL_NAME,COURSE)
VALUES
("JAKE","DIPLOMA"),
("MADDY","FASH");

#DELETE A ROW
DELETE FROM TABLE_1
WHERE DOB IS NULL;

#UPDATE AN ENIRE COLUMN 
UPDATE TABLE_1
SET DOB = NULL;  #WILL CHANGE ALL THE DOB ENTRIES TO NULL

SELECT * FROM TABLE_1;

# *************************************************************************************************
CREATE TABLE TABLE_2
(TODAY_DATE DATE,
TODAT_DATE_TIME DATETIME,
TODAY_TIME TIMESTAMP
);

INSERT INTO TABLE_2
VALUES
(current_date(),now(),current_timestamp());

SELECT * FROM TABLE_2;

INSERT INTO TABLE_2
VALUE
(current_date(),now(),current_timestamp());

#*******************************************************************************************************
-- DDL practice CREATE ,ALTER , DROP
CREATE TABLE TABLE_3
(DATE_TIME DATETIME DEFAULT NOW(),
S_NAME VARCHAR(20),
S_DOB DATE,
PHONE BIGINT
);

SHOW TABLES;
SHOW TABLE STATUS;

# Adding Unique Constrain
ALTER TABLE TABLE_2
ADD CONSTRAINT
UNIQUE (TODAY_DATE_TIME);


# setting up check constraint
ALTER TABLE TABLE_2
ADD CONSTRAINT VALUE_CHECK CHECK (VALUE>=1000.00 AND VALUE<=9999.99);


#default constraint
ALTER TABLE TABLE_2
ALTER VALUE SET DEFAULT 1000.00;

-- DELETING A UNIQUE CONSTRAINT FROM A TABLE
ALTER TABLE TABLE_1
DROP INDEX PHONE_NUMBER;

-- Assiging a PK in the Table_1
ALTER TABLE TABLE_1
ADD CONSTRAINT
PRIMARY KEY(FULL_NAME,PHONE_NUMBER);

-- Updating the DataType of a column
ALTER TABLE FUN_TABLE
MODIFY COLUMN S_NAME VARCHAR(50);

-- Adding Foreign Key , To the Fun_table which references the PK of Table_1
ALTER TABLE FUN_TABLE
ADD CONSTRAINT MY_FK
FOREIGN KEY(MOBILE_NO,S_NAME) REFERENCES TABLE_1(PHONE_NUMBER,FULL_NAME);

-- *********************************************************************************************
USE TEST_AND_FUN;

SHOW TABLES;
SHOW TABLE STATUS;

ALTER TABLE TABLE_3
ADD COLUMN TOKEN_NUMBER INT NOT NULL AUTO_INCREMENT AFTER PHONE,
ADD PRIMARY KEY(TOKEN_NUMBER);

ALTER TABLE TABLE_3
AUTO_INCREMENT = 155;

CREATE VIEW PAF_AMT_VIEW AS
SELECT PAF_AMOUNT
FROM BTECH_ADMISSION
WHERE PAF_AMOUNT IS NOT NULL;

SELECT * FROM PAF_AMT_VIEW
ORDER BY PAF_AMOUNT DESC;

-- FIND THE 2ND MOST LARGET PAF_AMOUNT IN THE TABLE
SELECT PAF_AMOUNT
FROM PAF_AMT_VIEW
WHERE PAF_AMOUNT <>
(SELECT MAX(PAF_AMOUNT)
FROM PAF_AMT_VIEW)
ORDER BY PAF_AMOUNT DESC
LIMIT 1;

-- ************************** ASSIGNMENT ON TASK_TABLE ****************************************************
SELECT * FROM TASK_TABLE;

-- 1. Rank Employees by Performance (Overall)
SELECT EMP_ID,NAME,
DENSE_RANK() OVER (ORDER BY PERF_SCR DESC) AS RANKING_VIA_PERFORMANCE
FROM TASK_TABLE
ORDER BY PERF_SCR DESC;

-- 2. Rank Employees Within Each Department by Salary
SELECT EMP_ID,NAME,DEPT,
DENSE_RANK() OVER (PARTITION BY DEPT ORDER BY SALARY DESC) AS RANKING_WITHIN_DEPARTMENT_VIA_SALARY
FROM TASK_TABLE
ORDER BY DEPT;

-- 3. Identify Top 2 Salaried Employees per Department
SELECT * FROM
(SELECT EMP_ID,NAME,DEPT,
DENSE_RANK() OVER (PARTITION BY DEPT ORDER BY SALARY DESC) AS RANKING_WITHIN_DEPARTMENT_VIA_SALARY
FROM TASK_TABLE
ORDER BY DEPT) RANK_TABLE
WHERE RANKING_WITHIN_DEPARTMENT_VIA_SALARY<=2
ORDER BY DEPT;

-- 4. Find All Employees with Tied Salaries in IT
SELECT *
FROM TASK_TABLE
WHERE DEPT = 'IT'
  AND SALARY IN (
    SELECT SALARY
    FROM TASK_TABLE
    WHERE DEPT= 'IT'
    GROUP BY SALARY
    HAVING COUNT(*) > 1
);

-- ********* FIND THE Nth Highest Salary **********
-- Finding the 6th Highest Salary , if there are multiple people with 6th Highest Salary
-- PART 1 : RETURN ALL THEIR NAMES AND EMP_ID || PART 2 : RETURN ONLY THAT EMPLOYEE NAME AND EMP_ID WHOSE EMP_ID IS THE GREATEST

-- PART 1 SOLUTION QUERRY
SELECT EMP_ID,NAME
FROM
(SELECT EMP_ID,NAME,
DENSE_RANK() OVER (ORDER BY SALARY DESC) AS RNK_VIA_SALARY
FROM TASK_TABLE) TEMP_TABLE
WHERE RNK_VIA_SALARY=6;

-- PART 2 SOLUTION QUERRY
SELECT EMP_ID,NAME
FROM
(SELECT EMP_ID,NAME,
DENSE_RANK() OVER (ORDER BY SALARY DESC) AS RNK_VIA_SALARY
FROM TASK_TABLE) TEMP_TABLE
WHERE RNK_VIA_SALARY=6
ORDER BY EMP_ID DESC
LIMIT 1;


SELECT * FROM FUN_TABLE;
USE ADMISSION;
SHOW TABLES;

SELECT * FROM TASK_TABLE;

DELIMITER $$
CREATE PROCEDURE SIX_TH_HIGHEST_SALARY()
BEGIN
	SELECT EMP_ID,NAME
	FROM
	(SELECT EMP_ID,NAME,
	DENSE_RANK() OVER (ORDER BY SALARY DESC) AS RNK_VIA_SALARY
	FROM TASK_TABLE) TEMP_TABLE
	WHERE RNK_VIA_SALARY=6;
END $$
DELIMITER ;

CALL SIX_TH_HIGHEST_SALARY();


-- ****************** STORED PROCEDURE WITH INPUT **********************
DELIMITER $$
CREATE PROCEDURE N_TH_HIGHEST_SALARY(IN TARGET INT)
BEGIN
	SELECT EMP_ID,NAME
	FROM
	(SELECT EMP_ID,NAME,
	DENSE_RANK() OVER (ORDER BY SALARY DESC) AS RNK_VIA_SALARY
	FROM TASK_TABLE) TEMP_TABLE
	WHERE RNK_VIA_SALARY=TARGET;
END $$
DELIMITER ;

CALL N_TH_HIGHEST_SALARY(14);

-- ***********************************************
USE ADMISSION;

SHOW TABLES;

SELECT * FROM COURSE_DETAILS;
SELECT * FROM STUDENT_DETAILS;

SELECT *
FROM COURSE_DETAILS AS CD
INNER JOIN
STUDENT_DETAILS AS SD
ON CD.COURSE_ID=SD.COURSE_ID;


-- ***********************************************************************
SELECT *
FROM payments;

# Add a new column to the PAYMENTS TABLE AS type
ALTER TABLE PAYMENTS
ADD COLUMN P_type varchar(10);

UPDATE payments
SET AMOUNT=700
WHERE P_id=1;

SELECT current_date();  # pRINTS THE CURRENT-TODAYS DATE
SELECT current_time();  # Prints the current time
SELECT current_timestamp(); #Prints current date - time (but preffer this more)
SELECT current_user();
SELECT now(); #Prints current date - time

ALTER TABLE PAYMENTS
ADD COLUMN UID VARCHAR(3);

ALTER TABLE PAYMENTS
ADD CONSTRAINT
UNIQUE(UID);

ALTER TABLE PAYMENTS
DROP INDEX UID;

-- ****** NULL , NOT NULL CONSTARINT *****
ALTER TABLE PAYMENTS
MODIFY COLUMN P_id int NOT NULL;

-- Dropping the Null constraint
ALTER TABLE PAYMENTS
MODIFY COLUMN P_id int NULL;

-- aDDING THE CHECK CONSTRAINT IN THE PAYEMENTS Table
ALTER TABLE PAYMENTS
ADD CONSTRAINT amt_validator
CHECK (Amount>=0.00 AND Amount<=1000.00);

INSERT INTO payments (P_id,Amount)
VALUES
(12,500),
(14,50);

ALTER TABLE PAYMENTS
DROP CONSTRAINT amt_validator;

-- Default Constraint

CREATE TABLE IF NOT EXISTS XYZ
( Eid VARCHAR(3) NOT NULL,
  Name VARCHAR(20) NOT NULL DEFAULT "Nan",
  Phone BIGINT NOT NULL DEFAULT -999,
  TODAY_DATE DATETIME DEFAULT CURRENT_TIMESTAMP()
);

INSERT INTO XYZ(Eid)
VALUES
(124),
(134),
(240);

SELECT * FROM XYZ;
SELECT * FROM PAYMENTS;

ALTER TABLE PAYMENTS
ALTER Amount SET DEFAULT 100,
ALTER P_type SET DEFAULT "CASH";

-- Adding PK to the XYZ table
ALTER TABLE XYZ
ADD CONSTRAINT pk_key
PRIMARY KEY (Eid);

ALTER TABLE XYZ
DROP PRIMARY KEY;

CREATE TABLE IF NOT EXISTS ABIR
(Eid VARCHAR(3) PRIMARY KEY,
PHONE BIGINT NOT NULL DEFAULT 99999);

CREATE TABLE IF NOT EXISTS MAZ
(Cid VARCHAR(3) PRIMARY KEY,
M_id VARCHAR(3) NOT NULL);


ALTER TABLE MAZ
MODIFY COLUMN M_id VARCHAR(3) NULL;

ALTER TABLE MAZ
ADD CONSTRAINT MAZ_FK
FOREIGN KEY(M_id) REFERENCES ABIR(Eid)
ON DELETE SET NULL;

SELECT * FROM ABIR;
SELECT * FROM MAZ;

DELETE FROM ABIR
WHERE Eid=200;

SHOW COLUMNS FROM PAYMENTS;

DESCRIBE PAYMENTS;

ALTER TABLE PAYMENTS
MODIFY COLUMN Amount BIGINT;

SELECT current_timestamp();

USE ADMISSION;

SELECT current_timestamp;

SELECT COUNT()
FROM PAYMENTS;